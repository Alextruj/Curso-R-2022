...
fit_pib_2 <- lm(PIB ~ lagPIB+Construccion+Ind_manufacturas,
                data = datos_subset)
...
#La primera ventana empieza en el indice 9 (1995/01) 
...
#Un nuevo data frame sin NA's
datos_reg <- select(tasa_anual, PIB, lagPIB, Construccion, Ind_manufacturas)
datos_reg <- datos_reg %>% na.exclude()

#Ahora si la regresion
out <- roll_regres(PIB ~ lagPIB+Construccion+Ind_manufacturas,
                   data = datos_reg, width = 32)
