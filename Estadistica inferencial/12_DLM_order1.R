...
model <- dlmModPoly(order = 1, dV = 1, dW = 0.01, m0 = 5, C0 = 10)

#Para comprobar que, en efecto tenga los argumentos correctos
FF(model)
GG(model)
V(model)
W(model)

#Puedes probar despues con esta para confirmar que es lo mismo
#model <- dlm(FF = 1, V = 1, GG = 1, W = 0.01, m0 = 5, C0 = 10)
...
#Pronosticos
#La funcion dlmForecast crea observaciones futuras a partir de la distribucion
## posterior, y las ecuaciones de estado

#Le pediremos 8 trimestres, y 100 muestras
fore <- dlmForecast(model, nAhead=8, sampleNew = 100)

#Vamos a arreglar la lista que nos dio
sample <- unlist(fore$newObs) %>% matrix(nrow = 8, 100)
sample


#Saquemos los cuantiles
